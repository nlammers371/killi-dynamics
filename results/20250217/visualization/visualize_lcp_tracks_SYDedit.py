...
if __name__ == "__main__":
    ...
    class TrackFilterWidget(QWidget):
        def __init__(self, viewer, track_df, name_prefix="highlighted", color="red"):
            super().__init__()
            self.viewer = viewer
            ...
            self.layout().addWidget(QLabel("Enter Track ID to Highlight:"))

            self.input = QLineEdit()
            self.layout().addWidget(self.input)

            self.button = QPushButton("Highlight Track")
            self.layout().addWidget(self.button)

            self.button.clicked.connect(self.highlight_track)

        def highlight_track(self):
            try:
                track_id = int(self.input.text())
            except ValueError:
                print("Invalid input, please enter an integer.")
                return

            if track_id not in self.track_df["track_id"].values:
                print(f"Track ID {track_id} not found.")
                return

            # Remove previous highlight layer(s)
            for layer in self.viewer.layers:
                if layer.name.startswith(self.name_prefix):
                    self.viewer.layers.remove(layer)

            track_data = self.track_df[self.track_df["track_id"] == track_id][["track_id", "t", z_var, "y", "x"]]
            self.viewer.add_tracks(
                track_data,
                name=f"{self.name_prefix}_{track_id}",
                scale=scale_vec,
                color=self.color,
                tail_width=6,
                tail_length=999,
            ...
            print(f"Track {track_id} highlighted.")

    viewer.window.add_dock_widget(TrackFilterWidget(viewer, nls_tracks_df, name_prefix='highlight_nls', color = "magenta"), area = 'right')
    viewer.window.add_dock_widget(TrackFilterWidget(viewer, lcp_tracks_df, name_prefix='highlight_lcp', color = "yellow"), area = 'right')
