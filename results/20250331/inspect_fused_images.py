...
def write_napari_frames(i, viewer, image_full, mask_full, layer1, layer0, initial_angle, output_dir,
                        scale_vec, angle_increment, X, Y, Z,
    ...
    if dist_mask is None:
        ...
        C = fitted_center
        # Create coordinate grids in a memory-efficient way using np.ogrid
    ...
    if i < 1000:
        ...
        lcp_o = (i - 1000) / 100 * 0.6  # linearly increase opacity from 0.01 to 1.0 over 100 frames
        lcp_norm[sdf > 30] = 0
    else:
        lcp_o = 0.6
        lcp_norm[sdf > 30] = 0
    ...
    viewer.camera.angles = tuple([initial_angle[0], initial_angle[1] + i * angle_increment, initial_angle[2]])
...
if __name__ == "__main__":
    ...
    zpath = os.path.join(d_root, "built_data", "zarr_image_files", project + "_fused.zarr")
    ...
    # make output path
    output_dir = os.path.join(root, "figures", project, "screenshots_h3", "")
    ...
    #
    # for key in list(mask.attrs.keys()):
    #     image_full.attrs[key] = mask.attrs[key]

    # get scale info
    scale_vec = tuple([image_full.attrs['PhysicalSizeZ'], image_full.attrs['PhysicalSizeY'], image_full.attrs['PhysicalSizeX']])

    # Assume the time axis is the first dimension of your data
    ...
    layer0 = viewer.add_image(image_full[0, 1], name='lcp', rendering='mip', scale=scale_vec, contrast_limits=[20, 850])
    layer1 = viewer.add_image(image_full[0, 0], name='lcp', rendering='mip', colormap='green', scale=scale_vec,
                              contrast_limits=[135, 275], opacity=0.6)
    ...
    # Store the initial camera roll so we have a reference (if needed)
    initial_angle = (-3.8195498904405216, 8.89243987179023, 105.88077856425232)#viewer.camera.angles
    ...
    #     # process_map(
    #     #     run_write_napari,
    #     #     range(0, num_frames, frame_increment),
    #     #     max_workers=n_workers,  # Adjust based on your CPU cores
    #     #     chunksize=1,  # Process one frame at a time for better memory management
    #     # )
    # else:
    toggle = False
    for frame in tqdm(range(start_i, stop_i, frame_increment), f"Writing frames from {start_i} to {stop_i}"):
        """
