...
def load_split_train_test(datadir, model_type, model_config, training_config, valid_size=.2, train_frac=0.5):
    ...
    if model_type == "MetricVAE":
        ...
        # Make datasets
        train_data = MyCustomDataset(root=os.path.join(datadir),
        ...
        if model_config.metric_loss_type == "NT-Xent":
            # Make datasets
            train_data = SeqPairDataset(root=os.path.join(datadir),
            ...
            # Make datasets
            train_data = TripletPairDataset(root=os.path.join(datadir),
                                           model_config=model_config,
                                           mode="train",
                                           transform=data_transform,
                                           return_name=True
    ...
    # train_data_rs = [train_data[i] for i in train_idx[:100]]
    # eval_data_rs = eval_data[eval_idx]

    train_sampler = SubsetRandomSampler(train_idx)
    eval_sampler = SubsetRandomSampler(eval_idx)

    trainloader = torch.utils.data.DataLoader(train_data, sampler=train_sampler, batch_size=training_config.per_device_train_batch_size)
    evalloader = torch.utils.data.DataLoader(eval_data, sampler=eval_sampler, batch_size=training_config.per_device_eval_batch_size)
...
class ContrastiveLearningDataset:
    ...
    def get_simclr_pipeline_transform():#(size, s=1):
        ...
        data_transforms = transforms.Compose([transforms.Grayscale(num_output_channels=1),
                                              transforms.RandomAffine(degrees=90, scale=tuple([0.7, 1.3])),
